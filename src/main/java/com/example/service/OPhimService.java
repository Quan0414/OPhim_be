...
import com.example.client.OPhimGateway;
...
public class OPhimService {
    ...
    private final OPhimGateway gateway;

    @Inject
    public OPhimService(OPhimGateway gateway) {
    ...
    public JsonNode search(String keyword, Integer page, Integer limit) {
        if (keyword == null || keyword.trim().length() < 2) {
            throw new OPhimException(400, "keyword must contain at least 2 characters");
        }
    ...
    private void validateSlug(String slug, String name) {
        if (slug == null || !SLUG_PATTERN.matcher(slug).matches()) {
            throw new OPhimException(400, name + " is invalid");
        }
    }

    private void validateYear(String year) {
        if (year == null || !YEAR_PATTERN.matcher(year).matches()) {
            throw new OPhimException(400, "year must contain 4 digits");
        }
    }

    private void validateRange(Integer value, String name, int min, int max) {
        if (value == null || value < min || value > max) {
            throw new OPhimException(400, name + " must be between " + min + " and " + max);
        }
    }
